      ...
      <template #form>
        ...
        <div v-if="twoFactorEnabled">
          <div v-if="qrCode">
            <div class="form_text_info">
              <p v-if="confirming" class="font-semibold">
                Um die Aktivierung der Zwei-Faktor-Authentifizierung abzuschließen,
                scannen Sie den folgenden QR-Code mit der Authentifizierungsanwendung Ihres Smartphones
                oder geben Sie den Einrichtungsschlüssel ein und geben Sie den generierten OTP-Code ein.
              </p>

              <p v-else>
                Die Zwei-Faktor-Authentifizierung ist jetzt aktiviert. Scannen Sie den folgenden QR-Code
                mit der Authentifizierungsanwendung Ihres Smartphones oder geben Sie den Einrichtungsschlüssel ein.
              </p>
            </div>
          ...
          <div v-if="recoveryCodes.length > 0 && !confirming">
            <div class="form_text_info">
              Speichern Sie diese Wiederherstellungscodes in einem sicheren Passwort-Manager. Sie können verwendet werden,
              um den Zugang zu Ihrem Konto wiederherzustellen, wenn Ihr Gerät zur Zwei-Faktor-Authentifizierung verloren geht.
            </div>
        ...
        <div class="mt-5">
          ...
          <div v-else>
            <button-group>
              <confirms-password @confirmed="confirmTwoFactorAuthentication" v-if="confirming">
                <input-button
                  type="button"
                  class="me-3"
                  :class="{ 'opacity-25': enabling }"
              ...
              >
                <input-button class="me-3">Wiederherstellungs-Codes regenerieren</input-button>
              </confirms-password>

              <confirms-password @confirmed="showRecoveryCodes" v-if="recoveryCodes.length === 0 && !confirming">
                <input-button class="me-3">Wiederherstellungscodes anzeigen</input-button>
              </confirms-password>

              <confirms-password @confirmed="disableTwoFactorAuthentication" v-if="confirming">
                <input-button :class="{ 'opacity-25': disabling }" :disabled="disabling">
                  Abbrechen
                </input-button>
              </confirms-password>

              <confirms-password @confirmed="disableTwoFactorAuthentication" v-if="!confirming">
                <input-danger-button :class="{ 'opacity-25': disabling }" :disabled="disabling">
                  Deaktivieren
                </input-danger-button>
              </confirms-password>
            </button-group>
    ...
    methods: {
        ...
        router.post(
            ...
            {},
            {
                preserveScroll: true,
                onSuccess: () => {
                    // Zeige QR-Code, Setup Key und Recovery Codes
                    Promise.all([this.showQrCode(), this.showSetupKey(), this.showRecoveryCodes()]);
                    this.twoFactorEnabled = true;
                    this.confirming = true; // Bestätigung für OTP
                },
                onFinish: () => {
                    this.enabling = false;
                },
